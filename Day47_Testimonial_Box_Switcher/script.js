...
function updateTestimonial() {
    ...
    document.querySelector('.user').style.animation = 'none';
    ...
    userImage.src = photo;
    userImage.alt = `${name} - ${position}`;
    userName.textContent = name;
    role.textContent = position;

    testimonial.style.animation = 'fadeIn 0.5s ease forwards';
    document.querySelector('.user').style.animation = 'fadeIn 0.5s ease 0.2s forwards';
...
function goToPrevious() {
    stopAutoplay();
    idx = idx - 2;
    if (idx < 0) idx = testimonials.length - 2;
    updateTestimonial();
    startAutoplay();
}
